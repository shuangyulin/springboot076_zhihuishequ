<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="题目数" prop="timushu">
          <el-input v-model="ruleForm.timushu" 
              placeholder="题目数" clearable  :readonly="ro.timushu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="题目数" prop="timushu">
              <el-input v-model="ruleForm.timushu" 
                placeholder="题目数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info'" label="答卷" prop="dajuan">
          <file-upload
          tip="点击上传答卷"
          action="file/upload"
          :limit="1"
          :multiple="true"
          :fileUrls="ruleForm.dajuan?ruleForm.dajuan:''"
          @change="dajuanUploadChange"
          ></file-upload>
        </el-form-item>  
        <div v-else>
          <el-form-item v-if="ruleForm.dajuan" label="答卷" prop="dajuan">
            <el-button type="text" size="small" @click="download(ruleForm.dajuan)">下载</el-button>
          </el-form-item>
        </div>    
      </el-col>      
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="答卷内容" prop="dajuanneirong">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="答卷内容"
                  v-model="ruleForm.dajuanneirong" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.dajuanneirong" label="答卷内容" prop="dajuanneirong">
                    <span>{{ruleForm.dajuanneirong}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.leixing = true;
          ...
          if(o=='timushu'){
            this.ruleForm.timushu = obj[o];
	    this.ro.timushu = true;
            continue;
          }
          if(o=='fujian'){
	    ...
	    this.ro.xingming = true;
          ...
          if(o=='dajuanneirong'){
            this.ruleForm.dajuanneirong = obj[o];
	    this.ro.dajuanneirong = true;
            continue;
          }
          if(o=='dajuan'){
            this.ruleForm.dajuan = obj[o];
	    this.ro.dajuan = true;
            continue;
          }
        }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `canyudajuan/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.canyudajuanCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    dajuanUploadChange(fileUrls) {
	this.ruleForm.dajuan = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
